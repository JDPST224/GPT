...
func (pm *ProxyManager) Start() {
    ...
    go func() {
        clearTicker := time.NewTicker(5 * time.Minute) // Clear failures every 5 minutes
        ...
        for {
            ...
            case <-clearTicker.C:
                pm.testedMutex.Lock()
                pm.testedFailure = make(map[string]struct{})
...
func (pm *ProxyManager) ReportFailure(proxy string) {
    ...
    // Only record failure without removing proxy
    pm.testedMutex.Lock()
...
func (pm *ProxyManager) mergeProxies(newList []string) {
    ...
    // Always append new proxies to maintain all, including failed ones
...
func (pm *ProxyManager) filterNewAndTest(rawList []string) []string {
    ...
    // Collect new proxies not in success cache
    var toTestAll []string
    for _, p := range rawList {
        if _, ok := successCache[p]; ok {
            continue
        }
        // Removed failure cache check to allow retesting
